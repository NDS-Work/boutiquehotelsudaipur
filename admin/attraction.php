...
<head>
    ...
    <title><?= htmlspecialchars($pageTitle) ?> — Udaipur Admin</title>
...
<div class="page-header">
    <div>
        <h1>Attractions</h1>
        <p><?= count($attractions) ?> attraction<?= count($attractions) !== 1 ? 's' : '' ?> in Udaipur</p>
    </div>
    <button class="btn-add" onclick="openAdd()">
        <span style="font-size:1rem;line-height:1">＋</span> Add Attraction
    </button>
</div>
...
<div class="table-wrap">
    <?php if (empty($attractions)): ?>
        <div class="empty">No attractions found. Add one to get started.</div>
    <?php else: ?>
    <table>
        <thead>
            <tr>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="name"><?= htmlspecialchars($a['attraction_name']) ?></td>
                <td class="actions">
                    <button class="btn-edit"
                        onclick="openEdit(<?= $a['id'] ?>, <?= htmlspecialchars(json_encode($a['attraction_name'])) ?>)">
                        ✎ Edit
                    </button>
                    <button class="btn-del"
                        onclick="openDelete(<?= $a['id'] ?>, <?= htmlspecialchars(json_encode($a['attraction_name'])) ?>)">
                        ✕ Delete
                    </button>
                </td>
            </tr>
...
<div class="modal-bg" id="addModal">
    <div class="modal">
        ...
        <form method="POST" action="attraction.php">
            <input type="hidden" name="action" value="add">
            <div class="field">
                <label for="add_name">Attraction Name</label>
                <input type="text" id="add_name" name="attraction_name"
                       placeholder="e.g. City Palace" required>
            </div>
...
<div class="modal-bg" id="editModal">
    <div class="modal">
        ...
        <form method="POST" action="attraction.php">
            ...
            <div class="field">
                <label for="edit_new">New Attraction Name</label>
                <input type="text" id="edit_new" name="new_attraction_name"
    ...
    <div class="modal">
        ...
        <form method="POST" action="attraction.php">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn-submit danger">Delete</button>
            </div>
        </form>
    </div>
</div>
