...
<div class="page-header">
    <div>
        <h1>Occasions</h1>
        <p><?= count($occasions) ?> occasion<?= count($occasions) !== 1 ? 's' : '' ?></p>
    </div>
    <button class="btn-add" onclick="openAdd()">
        <span style="font-size:1rem;line-height:1">＋</span> Add Occasion
    </button>
</div>
...
<div class="table-wrap">
    <?php if (empty($occasions)): ?>
        <div class="empty">No occasions found. Add one to get started.</div>
    <?php else: ?>
    <table>
        <thead>
            <tr>
        ...
        <tbody>
            <?php foreach ($occasions as $i => $o): ?>
            <tr>
                <td class="num"><?= $i + 1 ?></td>
                <td class="name"><?= htmlspecialchars($o['occasion_name']) ?></td>
                <td class="actions">
                    <button class="btn-edit"
                            onclick="openEdit(<?= $o['id'] ?>, <?= htmlspecialchars(json_encode($o['occasion_name'])) ?>)">
                        ✎ Edit
                    </button>
                    <button class="btn-del"
                            onclick="openDelete(<?= $o['id'] ?>, <?= htmlspecialchars(json_encode($o['occasion_name'])) ?>)">
                        ✕ Delete
                    </button>
                </td>
            </tr>
...
<div class="modal-bg" id="addModal">
    <div class="modal">
        ...
        <h2>Add Occasion</h2>
        <form method="POST" action="occasion.php">
            <input type="hidden" name="csrf_token" value="<?= csrfToken() ?>">
            <input type="hidden" name="action" value="add">
            <div class="field">
                <label for="add_name">Occasion Name</label>
                <input type="text" id="add_name" name="occasion_name"
                       placeholder="e.g. Honeymoon Boutique Hotels" required>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn-cancel" onclick="closeModal('addModal')">Cancel</button>
                <button type="submit" class="btn-submit">Add Occasion</button>
            </div>
        </form>
    </div>
</div>
...
<div class="modal-bg" id="editModal">
    <div class="modal">
        ...
        <form method="POST" action="occasion.php">
            ...
            <div class="field">
                <label for="edit_new">New Occasion Name</label>
                <input type="text" id="edit_new" name="new_occasion_name"
